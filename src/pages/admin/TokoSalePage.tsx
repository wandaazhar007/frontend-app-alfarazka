...
export default function TokoSalePage() {
  ...
  const columns: TableColumn<CartItem & { index: number }>[] = [
    ...
    { key: 'subtotal', header: 'Subtotal', align: 'right', render: (item) => formatRupiah(item.qty * item.unitPrice) },
    {
      ...
      header: '',
      render: (item) => (
        <Button size="sm" variant="danger" onClick={() => removeFromCart(item.index)}>
          Hapus
        </Button>
      ),
    },
        ...
        <>
          <div className={styles.addForm}>
            <FormField label="Produk" htmlFor="toko-product">
              <select id="toko-product" value={selectedProductId} onChange={(e) => handleSelectProduct(e.target.value)}>
            ...
            <FormField label="Harga Satuan" htmlFor="toko-price">
              <input
                id="toko-price"
                type="number"
                value={unitPrice}
          ...
          <div className={styles.footerRow}>
            <div className={styles.paymentField}>
              <label htmlFor="toko-payment-method">Metode Bayar:</label>
              <select
            ...
            <Button variant="primary" onClick={handleSubmit} disabled={submitting || cart.length === 0}>
